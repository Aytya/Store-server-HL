...
func TestUpdateOrder(t *testing.T) {
	...
	user := domain.User{ID: 1}
	product := domain.Product{ID: 1}
	order := domain.Order{ID: 1, UserID: 1, ProductIDs: []uint{1}, TotalPrice: 100.0, Status: "new"}
	db.Create(&user)
	db.Create(&product)
	db.Create(&order)

	updatedOrder := domain.Order{TotalPrice: 150.0, Status: "processing"}
...
func TestSearchOrdersByUserID(t *testing.T) {
	db, cleanup := setupTestDB()
	defer cleanup()
	...
	req, _ := http.NewRequest(http.MethodGet, "/orders/search/user/1", nil)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = req
